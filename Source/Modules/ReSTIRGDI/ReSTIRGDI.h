...
namespace Falcor
{
    /** Implementation of ReSTIR for direct illumination.
        ...
        - Host:   Call ReSTIRGDI::beginFrame() on to begin a new frame.
        - Device: Populate surface data (GBuffer) using ReSTIRGDI::setSurfaceData()/setInvalidSurfaceData().

        For ReSTIR DI:

        - Host:   Call ReSTIRGDI::updateReSTIRDI() on to run the ReSTIR DI algorithm.
        - Device: Get final light samples using ReSTIRGDI::getFinalSample() and perform shading.

        Finally at the end of frame:
        - Host:   Call ReSTIRGDI::endFrame() to end the frame.
    */
    class ReSTIRGDI
    {
    public:
        ...
        */
        struct Options
        {
            ...
            bool useMFactor = false; // enable this will cause large variance for pairwise MIS
        ...
        */
        ReSTIRGDI(const ref<Scene>& pScene, const Options& options = Options(), const DefineList& ownerDefines = DefineList());
        ...
        */
        void updateReSTIRDI(RenderContext* pRenderContext, const ref<Texture>& pMotionVectors, const ref<Texture>& pViewDir = nullptr);
